...
10.1 Volcano plot (BG_4h vs RPMI_4h)
```{r}
res24 <- as.data.frame(res_BG_4h) %>%
...
ggplot(res24, aes(log2FoldChange, -log10(padj))) +
  ...
  theme_minimal() +
  labs(title = "Volcano: BG_4h vs RPMI_4h (DESeq2)",
       x = "log2 Fold Change", y = "-log10(padj)")
```

Heatmap top genes (Vst)
```{r}
...
10.2 Volcano plot (BG_24h vs RPMI_24h)
```{r}
